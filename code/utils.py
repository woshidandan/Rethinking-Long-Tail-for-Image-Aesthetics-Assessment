...
def simloss(features, targets):
    ...
    if targets.dim() == 2 and targets.size(1) == 10:
        # 10维打分分布
        y = torch.tensor(get_score(targets), device=features.device).unsqueeze(1)
    elif targets.dim() == 1:
        # 已经是分数（均值），直接使用
    ...
    denom = torch.clamp(torch.max(y) - torch.min(y), min=1e-6)
    ratio=(torch.abs(y-y.t()))/denom
    # label_sim_mat = 1-hyper1*(ratio**hyper2)
    label_sim_mat = 1-1*(ratio**1)
    label_sim_mat = label_sim_mat.to(features.device)
