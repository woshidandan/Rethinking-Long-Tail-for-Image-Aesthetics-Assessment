...
class AVADataset(Dataset):
    def __init__(self, csv_path, dataset_path,self_training=False,mode='train',loss_type='emd',
                 generated_dataset_path=None, pseudo_logits_path='logits.csv',
                 pseudo_results_path='test_result.csv'):
        self.df = pd.read_csv(csv_path)
        self.dataset_path = dataset_path
        self.generated_dataset_path = generated_dataset_path
        self.pseudo_logits_path = pseudo_logits_path
        self.pseudo_results_path = pseudo_results_path
        self.mode=mode
    ...
    def __getitem__(self, item):
        ...
        try:
            image = default_loader(os.path.join(self.dataset_path, image_id))
        except Exception:
            if not self.generated_dataset_path:
                raise
            category = str(row['category']) if 'category' in row else ''
            image_path = os.path.join(self.generated_dataset_path, category, image_id)
            if not os.path.exists(image_path):
                image_path = os.path.join(self.generated_dataset_path, image_id)
        ...
        if self.loss_type=='emd':
            ...
            if 'score' in row:
                ...
                scores=self.distribution_to_score(row)/10
    ...
    def distribution_to_score(self, row):
        scores_names = [f'score{i}' for i in range(2, 12)]
        scores = np.array([row[k] for k in scores_names], dtype=np.float32)
        scores = scores / scores.sum()
        return float(np.sum(scores * np.arange(1, 11)))
    ...
    def grid_search(self):
        ...
        best_mae=1e6
        best_beta, best_threshold = beta_choices[0], threshold_choices[0]
    ...
    def pick_pseudo_labels(self,beta=1.0,threshold=0.80):
        ...
        '''
        logits = pd.read_csv(self.pseudo_logits_path)
        labels = pd.read_csv(self.pseudo_results_path)
